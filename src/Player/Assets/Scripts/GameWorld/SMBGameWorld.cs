...
public class SMBGameWorld : SMBSingleton<SMBGameWorld> {
	...
	private bool isLevelValid() {
		...
		if (playerAmount != 1) {
			UnityEngine.Debug.LogError ("This level does not have a player. Levels must have one and only one player.");
			return false;
		}

		endPoleAmount = 1;
		if (endPoleAmount != 1) {
			UnityEngine.Debug.LogError ("This level does not have an end. Levels must have one and only one end.");
			return false;
		}
